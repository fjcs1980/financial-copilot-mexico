...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({
  ...
  return (
    <section aria-labelledby="results-heading" className="max-w-2xl w-full mx-auto p-6 bg-white rounded-xl shadow-md space-y-6">
      <div>
      ...
      {income > 0 && (
        <div className="space-y-1">
          <div
            role="progressbar"
            aria-label="Distribución proporcional del ingreso"
            aria-valuenow={Math.min(100, essentialPct + bufferPct + debtPct)}
            aria-valuemin={0}
            aria-valuemax={100}
            className="w-full bg-gray-200 h-2.5 rounded-full overflow-hidden flex"
          >
            <div
          ...
          <div className="flex justify-between text-[10px] text-gray-400 px-1" aria-hidden="true">
      ...
      {status === 'NO_MONEY_FOR_DEBT' && (
        <div role="alert" className="p-4 bg-amber-50 border-l-4 border-amber-500 rounded-r-md">
      ...
      {status === 'PAYMENT_PLAN_AVAILABLE' && (
        <div role="status" className="p-4 bg-emerald-50 border-l-4 border-emerald-500 rounded-r-md">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-gray-700 border border-gray-200 rounded-lg overflow-hidden">
            <thead className="bg-gray-100 text-gray-600 uppercase font-semibold">
              <tr>
                <th scope="col" className="p-3">Deuda</th>
                <th scope="col" className="p-3">Saldo Actual</th>
                <th scope="col" className="p-3">Pago Mínimo</th>
                <th scope="col" className="p-3">Pago Extra</th>
                <th scope="col" className="p-3 text-emerald-700">Pago Sugerido</th>
              </tr>
            </thead>
      ...
      <div className="flex justify-between pt-4 border-t">
        <button
          type="button"
          onClick={onBack}
          className="px-4 py-2 bg-gray-200 text-gray-700 rounded-md text-sm font-medium hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 transition cursor-pointer"
        >
          Modificar Deudas
        </button>
        <button
          type="button"
          onClick={onReset}
          className="px-6 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition cursor-pointer"
        >
          Nuevo Cálculo
        </button>
      </div>
